...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
...
#endif
        OS_ENTER_CRITICAL();
        printf("当前系统时间:%d, 任务%d正在进行计算\n", OSTime, OSPrioCur);
        if (OSTCBCur->OSTCBExtPtr != (void*)0) {
            printf("任务%d还需计算时间:%d，剩余周期时间:%d\n", OSTCBCur->OSTCBPrio, ((TaskExt*)OSTCBCur->OSTCBExtPtr)->c, ((TaskExt*)OSTCBCur->OSTCBExtPtr)->t);
            --(((TaskExt*)OSTCBCur->OSTCBExtPtr)->c);
        }
        OS_EXIT_CRITICAL();
        ...
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list              */
            OS_ENTER_CRITICAL();
            if (ptcb->OSTCBExtPtr != (void*)0 && ptcb->OSTCBDly == 0) {
                if (((TaskExt*)ptcb->OSTCBExtPtr)->t > 0) {
                    --(((TaskExt*)ptcb->OSTCBExtPtr)->t);
                }
                if (((TaskExt*)ptcb->OSTCBExtPtr)->t <= 0) {
                    if (((TaskExt*)ptcb->OSTCBExtPtr)->c <= 0) {
                        printf("任务%d运算周期结束，开始新周期\n", ptcb->OSTCBPrio);
                    }
                    else {
                        printf("任务%d超时未完成，取消本次计算", ptcb->OSTCBPrio);
                    }
                    ((TaskExt*)ptcb->OSTCBExtPtr)->c = ((TaskExt*)ptcb->OSTCBExtPtr)->C;
                    ((TaskExt*)ptcb->OSTCBExtPtr)->t = ((TaskExt*)ptcb->OSTCBExtPtr)->T;
                    ((TaskExt*)ptcb->OSTCBExtPtr)->d = OSTime + ((TaskExt*)ptcb->OSTCBExtPtr)->D;
                }
            }
            ...
            if (ptcb->OSTCBExtPtr != (void*)0 && ptcb->OSTCBDly == 0) {
                if (((TaskExt*)ptcb->OSTCBExtPtr)->d == 0) {
                    ((TaskExt*)ptcb->OSTCBExtPtr)->d = OSTime + ((TaskExt*)ptcb->OSTCBExtPtr)->D;
                }
            }
...
static  void  OS_SchedRM(void)
{
    ...
    INT16U prio = OS_TASK_IDLE_PRIO;
    INT32S mint = OS_TASK_MAX_PERIOD;
    while (ptcb != (void*)0) {
        if (ptcb->OSTCBStat == OS_STAT_RDY && ptcb->OSTCBDly == 0) {
            if (ptcb->OSTCBExtPtr != (void*)0) {
                if (((TaskExt*)ptcb->OSTCBExtPtr)->c > 0) {
                    prio = ((TaskExt*)ptcb->OSTCBExtPtr)->T < mint ? ptcb->OSTCBPrio : prio;
                    mint = ((TaskExt*)ptcb->OSTCBExtPtr)->T < mint ? ((TaskExt*)ptcb->OSTCBExtPtr)->T : mint;
                }
            }
            else {
                if (mint == OS_TASK_MAX_PERIOD) {
                    prio = ptcb->OSTCBPrio < prio ? ptcb->OSTCBPrio : prio;
                }
            }
        }
        ptcb = ptcb->OSTCBNext;
...
{
    OS_TCB* ptcb = OSTCBList;
    OS_TCB* ctcb = OSTCBList;
    INT16U prio = OS_TASK_IDLE_PRIO;
    INT32U edl = OS_TASK_LATEST_DEADLINE;
    while (ptcb != (void*)0) {
        if (ptcb->OSTCBStat == OS_STAT_RDY && ptcb->OSTCBDly == 0) {
            if (ptcb->OSTCBExtPtr != (void*)0) {
                if (((TaskExt*)ptcb->OSTCBExtPtr)->c > 0) {
                    prio = ((TaskExt*)ptcb->OSTCBExtPtr)->d < edl ? ptcb->OSTCBPrio : prio;
                    edl = ((TaskExt*)ptcb->OSTCBExtPtr)->d < edl ? ((TaskExt*)ptcb->OSTCBExtPtr)->d : edl;
                }
            }
            else {
                if (edl == OS_TASK_LATEST_DEADLINE) {
                    prio = ptcb->OSTCBPrio < prio ? ptcb->OSTCBPrio : prio;
                }
            }
        }
