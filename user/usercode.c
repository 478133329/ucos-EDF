...
void mytestA(void* pParam)
{
	...
	while (1)
	{
		...
		OSSemQuery(MyEventSem, &mySemData);
		printf("时间:%d,任务A获得信号量。当前信号量值=%d，任务A开始对R操作\n", OSTimeGet(), mySemData.OSCnt);
		OSTimeDly(100); /*模拟操作资源,需要1秒，100个时钟嘀嗒*/
		printf("时间:%d，任务A结束资源操作，提交信号量！\n", OSTimeGet());
		OSSemPost(MyEventSem);
		OSSemQuery(MyEventSem, &mySemData);
		printf("时间:%d,任务A提交信号量完成，当前信号量值=%d,任务A将延时阻塞100嘀嗒\n", OSTimeGet(), mySemData.OSCnt);
		OSTimeDly(100);
	}
}
...
void mytestB(void* pParam)
{
	...
	while (1)
	{
		...
		OSSemQuery(MyEventSem, &mySemData);
		printf("时间:%d,任务B获得信号量。当前信号量值=%d，任务B开始对R操作,需100个时钟嘀嗒\n", OSTimeGet(), mySemData.OSCnt);
		OSTimeDly(100); /*模拟操作资源,需要1秒，100个时钟嘀嗒*/
		printf("时间:%d，任务B结束资源操作，提交信号量！\n", OSTimeGet());
		OSSemPost(MyEventSem);
		OSSemQuery(MyEventSem, &mySemData);
		printf("时间:%d,任务B提交信号量完成，当前信号量值=%d,任务B将延时阻塞100嘀嗒\n", OSTimeGet(), mySemData.OSCnt);
		OSTimeDly(100);
	}
}
...
void mytestC(void* pParam)
{
	...
	while (1)
	{
		...
		printf("时间:%d，任务C结束资源操作，提交信号量！\n", OSTimeGet());
		OSSemPost(MyEventSem);
		printf("时间:%d,任务C提交信号量完成，当前信号量值=%d,任务C将延时阻塞100嘀嗒\n", OSTimeGet(), mySemData.OSCnt);
		OSTimeDly(100);
	}
}
